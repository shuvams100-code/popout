...
export async function fetchPins(): Promise<Pin[]> {
  ...
  const [{ data: popouts }, { data: events }] = await Promise.all([
    supabase
      .from("popouts")
      .select("id,title,venue,lat,lng,starts_at,max_people,event_id,host:profiles!host_id(name,photo_url),members:popout_members(status)")
      .eq("status", "open")
      .gte("starts_at", from)
      .order("starts_at"),
  ...
  const pins: Pin[] = [
    ...(popouts ?? []).map((p) => ({
      ...
      host: { name: (p.host as unknown as { name: string; photo_url: string | null }).name, photo: (p.host as unknown as { photo_url: string | null }).photo_url },
      eventId: p.event_id,
    })),
    ...(events ?? []).map((e) => ({
      kind: "event" as const,
  ...
  return (soon.length ? soon : pins).slice(0, 60);
}
